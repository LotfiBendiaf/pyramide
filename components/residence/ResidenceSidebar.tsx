...
export default function ResidenceSidebar({ residence }: { residence: Residence }) {
  const ref = residence.referenceCode || "";
  ...
  const infoMsg = encodeURIComponent(
    `Bonjour, j'aimerais avoir plus d'informations sur la résidence ${residence.title}${ref ? ` (${ref})` : ""} à ${city}.`
  );
  const visitMsg = encodeURIComponent(
    `Bonjour, je souhaite planifier une visite pour la résidence ${residence.title}${ref ? ` (${ref})` : ""} à ${city}.`
  );

  const waVisit = `https://wa.me/${WHATSAPP_NUMBER}?text=${visitMsg}`;
  const waMessage = `https://wa.me/${WHATSAPP_NUMBER}?text=${infoMsg}`;

  const agentName = residence.agent
    ? `${residence.agent.firstname} ${residence.agent.lastname}`
    : undefined;

  return (
    <aside className="sticky top-24 space-y-6">
      <div className="rounded-2xl border p-6 space-y-4">
        <h3 className="text-xl font-semibold">Intéressé par cette résidence ?</h3>
      ...
      {agentName && (
        <div className="rounded-2xl border p-6 space-y-4">
          ...
          <div className="space-y-3">
            ...
            {residence.agent?.phone && (
              <div>
                <p className="text-xs text-muted-foreground mb-1">Téléphone</p>
                <a
                  href={`https://wa.me/${residence.agent.phone.replace(/\D/g, "")}`}
                  target="_blank"
            ...
            {residence.agent?.email && (
              <div>
                <p className="text-xs text-muted-foreground mb-1">Email</p>
                <a
                  href={`mailto:${residence.agent.email}`}
                  className="text-primary hover:underline flex items-center gap-2"
                >
        ...
        <div className="space-y-3 text-sm">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              >
                {residence.location.address && `${residence.location.address}, `}
          ...
          {residence.referenceCode && (
            <div className="flex items-center gap-2">
              <Badge variant="outline">{residence.referenceCode}</Badge>
            </div>
          )}
          ...
          {residence.deliveryDate && (
            <div>
              <p className="text-xs text-muted-foreground mb-2">Livraison prévue :</p>
              <p className="font-medium">{formatDate(residence.deliveryDate)}</p>
            </div>
          )}
        </div>
      </div>
