...
//   try {
//     // Get all agents with daily email enabled
//     const agents = await User.find({
//       role: { $in: ["AGENT", "MANAGER", "ADMIN"] },
//       "calendarSettings.dailyEmailEnabled": true,
//     });
...
//           const client = event.client as { firstName: string; lastName: string } | null;
...
//             type: event.sourceType === "FOLLOWUP" ? "followup" : event.sourceType === "TASK" ? "task" : "event",
...
//           const client = followUp.client as { firstName: string; lastName: string } | null;
...
//           const client = task.client as { firstName: string; lastName: string } | null;
...
//           calls: items.filter((i) => i.channel === "CALL" || i.channel === "WHATSAPP").length,
